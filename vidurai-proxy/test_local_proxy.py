"""
Test script for vidurai-proxy running locally
...
def test_memory_recall():
    ...
    if response.status_code == 200:
        ...
        if "blue" in response_text.lower() and ("memory" in response_text.lower() or "ai" in response_text.lower()):
            print("✅ Memory recall SUCCESSFUL!")
            return True
        else:
            print("⚠️ Memory recall failed - Claude doesn't remember")
            return False
    else:
        print(f"Error: {response.text}")
        return False
...
def test_metrics():
    ...
    if response.status_code == 200:
        ...
        # Check if metrics show activity
        if metrics.get('sessions', 0) > 0 or metrics.get('requests', 0) > 0:
            print("✅ Metrics tracking is working!")
            return True
        else:
            print("⚠️ Metrics show no activity")
            return False
    return False

def main():
    print("🚀 Testing Vidurai Proxy Locally")
    ...
    print("\n⏳ Waiting 2 seconds before memory recall test...")
    import time
    time.sleep(2)
    ...
    if total_passed == len(results):
        ...
        print(f"\n⚠️ {len(results) - total_passed} test(s) failed. Check logs above.")
